...
class BlockchainController {
    ...
    constructor(app, blockchainObj) {
        ...
        // aka an endpoint is not accessible unless declared in the constructor function.
        this.validateChain();
    ...
    // Endpoint to check blocks on the chain for valid / untampered data and
    // current block previous hash attribute matches actual previous block hash attribute (GET Endpoint)
    validateChain() {
        this.app.get("/blockchain/validateChain", async (req, res) => {
            let result[] = await this.blockchain.validateChain();
            if (result.length == 0) {
                return res.status(200).send("Chain is valid.");
            } else {
                return res.status(500).send("Chain is invalid!");
            }
        });
    }
